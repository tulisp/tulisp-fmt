...
/// Parse a token stream into a [`Cst`]. `source` is required because
/// the lexer leaves atom and comment text in the source — the parser
/// copies the relevant slices into the owned-`String` payloads on
/// `Atom` and `Comment` nodes.
pub fn parse(tokens: &[Token], source: &str) -> Result<Cst, ParseError> {
    let mut p = Parser {
        tokens,
        pos: 0,
        source,
...
mod tests {
    ...
    fn unclosed_paren_errors() {
        ...
        let err = parse(&tokens, "(foo").unwrap_err();
